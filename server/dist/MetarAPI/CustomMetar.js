...
import { Box, TextField, CircularProgress, Typography, Card, CardContent, Tooltip, Zoom, IconButton, Alert, } from "@mui/material";
...
function Metar() {
    ...
    const loading = (<Box display="flex" justifyContent="center" alignItems="center">
    ...
    async function searchIcao() {
        ...
        let metarListReduced = reduceTempo(metarList);
        let metarObj = maptoMetarObj(metarListReduced[0]);
        setMetarCode(metarObj);
        let flightRules = getFlightRules(metarObj.Visibility, metarObj.Cloud_Layer[0]?.cloudBase !== null
    ...
    return (<>
      <Card>
        <CardContent>
          <Box id="Metar text input ICAO" display="flex" flexDirection="column" justifyContent="center" alignItems="center">
            <Typography variant="h2">Metar</Typography>
            <form onSubmit={handleSubmit}>
              <TextField type="search" label="enter ICAO Code" value={icao} onChange={handleChange} InputProps={{
            endAdornment: (<IconButton id="searchButton" type="submit" onClick={handleSubmit} disabled={disabled}>
                      <Search />
                    </IconButton>),
        }}></TextField>
              {alertIcao && (<Alert severity="error">Please provide ICAO Code</Alert>)}
            </form>
          </Box>
          {isLoading && loading}
          {metarCode && (<Box id="Metar Data" display="flex" flexDirection="column" justifyContent="center" alignItems="center">
              ...
              <Tooltip title=<div>
                ...
                </div> arrow placement="right" TransitionComponent={Zoom}>
                <Typography style={{
