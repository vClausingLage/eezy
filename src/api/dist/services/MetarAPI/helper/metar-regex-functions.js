import { dateFormat, cloudFormat, windFormat, windVarFormat, tempFormat, precipFormat } from './metar-regex-helper-functions.js';
export function findDynamicTokens(metar) {
    const resultObj = {
...
export function findBasicTokens(metar) {
    const resultObj = {
        icao: '',
        date: undefined,
        cavok: false,
    ...
    const filteredMetar = metar.filter((el) => {
        if (/NOSIG/i.test(el))
            ...
            (resultObj.cavok = true), (resultObj.clouds[0].cloud_layer = 'CAVOK');
        }
